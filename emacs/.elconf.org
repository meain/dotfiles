...
** Line numbers
    ...
    (set-face-attribute 'fringe nil :background "#151515")
    #+END_SRC
** Cursor
    #+BEGIN_SRC emacs-lisp
...
**** Quit buffer
    #+BEGIN_SRC emacs-lisp
      ...
      (defun kill-current-buffer-unless-dashboard()
          (interactive)
          (if (= (length (mapcar #'window-buffer (window-list))) 1)
              (switch-to-buffer "*dashboard*")
              (evil-quit)))
      (evil-leader/set-key "q" 'kill-current-buffer-unless-dashboard)
      (define-key evil-normal-state-map (kbd "q") 'kill-current-buffer-unless-dashboard)
    #+END_SRC
...
*** Fiplr
    ...
    (use-package fiplr
       ...
       :config
           ...
           (setq fiplr-ignored-globs '((directories (".git" ".svn" "node_modules" "build" "dist"))
           (files ("*.jpg" "*.png" "*.zip" "*~"))))
       :init
       (progn
           ...
           (define-key evil-normal-state-map (kbd "<return>") 'recent-selector-open)))
    #+END_SRC
*** Recentf
    #+BEGIN_SRC emacs-lisp
    (use-package recentf
