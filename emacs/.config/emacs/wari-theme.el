;;; wari-theme.el --- Custom face theme for Emacs

;;; Code:
(defgroup wari nil "Custom faces for more stuff" :prefix "wari-")
(defvar wari-use-italics nil "Decides weather to use italics")

(deftheme wari "A minimal theme with simple colors")
...
(let* ((white "#FFFFFF")
       ...
       (commentary-layer `((t (:background ,lighter-gray :italic ,wari-use-italics))))
       (string-layer `((t (:background ,lighter-blue))))
       (defenition-layer `((t (:background ,lighter-orange))))
       (bold-layer `((t (:weight bold))))
  ...
  (defface wari-simple-gray `((t (:foreground ,medium-gray)))
    "Simple gray color for use outside comments"
    :group 'wari)
  (defface wari-all-white `,all-white-layer
    "Simple gray color for use outside comments"
    :group 'wari)

  (custom-theme-set-faces 'wari
    ;; Basic items
    ...
    ;; tree-sitter
    `(tree-sitter-hl-face:doc ,commentary-layer)
    `(tree-sitter-hl-face:string ,string-layer)
    `(tree-sitter-hl-face:method ,defenition-layer)
    `(tree-sitter-hl-face:function ,defenition-layer)
    ...
    `(tree-sitter-hl-face:constructor ,default-layer)
